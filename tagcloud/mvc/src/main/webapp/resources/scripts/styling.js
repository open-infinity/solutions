...
function resizeRightColumn() {
	$("#section").height($(window).height() - $("#header").height() - $("#footer").height() + "px");
	...
	if($(window).height() < $(window).width()) { // landscape display port
		...
		if($("#targetlist").outerHeight(true) < $("#section").height()) {
			$("#scroller").height($("#targetlist").outerHeight(true) + "px");
		} else {
			$("#scroller").height($("#section").height() + "px");
		}
...
function resizeMapView() {
	$("#map-canvas").height($("#section").height() + "px");
}

function setMapControls() {
	// zoom control
	var gmBase = $(".gm-style").first().css("left", $("#bc-col1").outerWidth(true) + "px").css("overflow", "visible");

	var gmBase = $(".gm-style > div").first().css("left", -($("#bc-col1").outerWidth(true)) + "px");
}
...
function resizeDirectionsColumn() {
	...
	var dircetionsScrollerHeight = ($("#section").outerHeight(true) - $("#directionGuidance").outerHeight(true)
			- $("#panel").outerHeight(true) - totalHeaderHeight);
	if($(window).height() >= $(window).width()) {
		dircetionsScrollerHeight -= $("#bc-col3").height();
	}
	$("#directionScroller").css("max-height", dircetionsScrollerHeight + "px");
}
