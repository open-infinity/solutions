...
function initialize() {
	...
	if (document.getElementById("latitude").getAttribute("value") > 0.0) {
		pos = new google.maps.LatLng(document.getElementById("latitude")
				.getAttribute("value"), document.getElementById("longitude")
				.getAttribute("value"));
	} else {
	...
	if (document.getElementById("map-zoom").getAttribute("value") > 0) {
		currentZoom = parseInt(document.getElementById("map-zoom")
				.getAttribute("value"));
	}
	...
	if (navigator.geolocation
			&& document.getElementById("latitude").getAttribute("value") == 0.0) {
	...
	google.maps.event.addListener(map, 'idle', function() {
		...
		setMapControls();
		populateCoordinates(map.getCenter().lat(), map.getCenter().lng(), map
				.getZoom());
		$("#searchModel").submit();
	});
				...
				.accordion(
						{
							activate : function(event, ui) {
								...
								var $activeTab = $("#accordion").accordion(
										'option', 'active');
	...
	$("#searchTextField").catcomplete(
			{
				...
				select : function(event, ui) {
					...
					if (ui.item.category == "Target") {
						window.location = "/tagcloud/target?target_id="
								+ ui.item.id;
					}
				}
			});
...
function isScrolledIntoView(elem, view) {
	...
	return ((elemBottom <= viewBottom) && (elemTop >= viewTop)
			&& (elemRight <= viewRight) && (elemLeft >= viewLeft));
}
